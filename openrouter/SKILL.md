...
- Credits / balance: `GET https://openrouter.ai/api/v1/credits` (management key)
- Per-model activity: `GET https://openrouter.ai/api/v1/activity` (**management key only**)
- Per-key spend: `GET https://openrouter.ai/api/v1/keys` (management key)
- Analytics meta (beta): `GET https://openrouter.ai/api/v1/analytics/meta`
- Analytics query (beta): `POST https://openrouter.ai/api/v1/analytics/query`

### Account, Analytics & Models reference
Full request/response shapes, key-type requirements, query params, and the
**completed-UTC-days lag** that makes `/activity` return empty for same-day spend:
see [references/account-analytics-endpoints.md](references/account-analytics-endpoints.md).

**Gotcha:** `/api/v1/activity` and the beta analytics endpoints require a
**management (provisioning) key** — a regular inference key returns
`403 "Only management keys can fetch activity for an account"`. And `/activity`
covers only the **last 30 _completed_ UTC days**; today's usage never appears
until the UTC day closes — an empty result with recent spend is expected, not a bug.
