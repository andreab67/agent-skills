...
description: Run this manually before ending a session — `/clear`, `/compact`, switching repos, or any context wipe — to rescue critical context that would otherwise die with the conversation. Trigger phrases include "about to /clear", "before I clear", "wrap up this session", "checkpoint before clearing", "hand off this session", "save context first", "preserve what we just figured out", "I'm switching repos", "/session-handoff". Extracts six categories — active project goals & scope, pending unresolved tasks with status, user preferences & constraints, established working rules, key background not in code/git, and open questions / agreed next steps — applies a single rescue test to each candidate (is this derivable from code, git, or CLAUDE.md? if yes, skip), writes survivors to the project's auto-memory store using the user/feedback/project/reference schema with consistent `name:` prefixes for retrieval, links related entries with `[[wiki-style]]` slugs, and requires explicit user approval per entry via AskUserQuestion before any file is written. Also skips secrets, ephemeral debug state, fixed-bug recipes, and anything already in a planning doc. Do NOT use this skill for ad-hoc single-fact memory saves during normal work (write the memory file directly — running the full extraction is overkill), for sessions that produced only code changes already captured in the diff and commit message, or after `/clear` has already run — at that point the context is gone, say so, and offer the optional hook setup for next time.
---
